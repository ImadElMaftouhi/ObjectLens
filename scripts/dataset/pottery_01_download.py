...
def _project_root() -> Path:
    # Resolve project root by searching upwards for a "data" or ".git" directory, fall back if not found.
    path = Path(__file__).resolve()
    for _ in range(5):
        if (path / "data").is_dir() or (path / ".git").is_dir():
            return path
        path = path.parent
    # Fallback if not found: go three levels up as before
...
def main() -> None:
    ...
    print("Tip: your groundtruth txt/xls files should be inside 3D_data/raw/ (or a subfolder).")
